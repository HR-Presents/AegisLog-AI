...
def test_v146_release_metadata_is_consistent():
    project = tomllib.loads(Path("pyproject.toml").read_text(encoding="utf-8"))
    init_text = Path("src/aegislog/__init__.py").read_text(encoding="utf-8")
    workflow = Path(".github/workflows/release-v1.4.6.yml").read_text(encoding="utf-8")
    notes = Path("docs/RELEASE_V1.4.6.md").read_text(encoding="utf-8")
    package_workflow = Path(".github/workflows/package.yml").read_text(encoding="utf-8")

    match = re.search(r'__version__\s*=\s*"([^"]+)"', init_text)

    assert project["project"]["version"] == "1.4.6"
    assert match and match.group(1) == "1.4.6"
    ...
    assert "aegislog_ai-1.4.6-py3-none-any.whl" in package_workflow
    assert "AegisLog-AI-v1.4.6-Customer-Bundle.zip" in package_workflow
