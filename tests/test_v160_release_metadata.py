...
def test_v200_release_metadata_is_consistent():
    project = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))
    init_text = (ROOT / "src/aegislog/__init__.py").read_text(encoding="utf-8")
    workflow = (ROOT / ".github/workflows/release-v2.0.0.yml").read_text(encoding="utf-8")
    ...
    notes = (ROOT / "docs/RELEASE_V2.0.0.md").read_text(encoding="utf-8")
    ...
    assert "RELEASE-v2.0.0" in workflow
    assert "AegisLog-v2.0.0-release-assets" in workflow
    assert "docs/RELEASE_V2.0.0.md" in workflow
    assert "Mission Control banner did not report v2.0.0" in workflow
    ...
    assert "aegislog_ai-2.0.0-py3-none-any.whl" in package_workflow
    assert "AegisLog-AI-v2.0.0-Customer-Bundle.zip" in package_workflow
    assert notes.startswith("# AegisLog v2.0.0")
