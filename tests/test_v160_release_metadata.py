...
def test_v190_release_metadata_is_consistent():
    project = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))
    init_text = (ROOT / "src/aegislog/__init__.py").read_text(encoding="utf-8")
    workflow = (ROOT / ".github/workflows/release-v1.9.0.yml").read_text(encoding="utf-8")
    ...
    notes = (ROOT / "docs/RELEASE_V1.9.0.md").read_text(encoding="utf-8")
    ...
    assert project["project"]["version"] == "1.9.0"
    assert match and match.group(1) == "1.9.0"
    assert "RELEASE_TAG: v1.9.0" in workflow
    assert "RELEASE_VERSION: 1.9.0" in workflow
    assert "RELEASE-v1.9.0" in workflow
    assert "AegisLog-v1.9.0-release-assets" in workflow
    assert "docs/RELEASE_V1.9.0.md" in workflow
    assert "Mission Control banner did not report v1.9.0" in workflow
    ...
    assert "aegislog_ai-1.9.0-py3-none-any.whl" in package_workflow
    assert "AegisLog-AI-v1.9.0-Customer-Bundle.zip" in package_workflow
    assert notes.startswith("# AegisLog v1.9.0")
