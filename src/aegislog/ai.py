...
def build_safe_prompt(context: InvestigationContext, limit: int = 80) -> str:
    ...
    return (
        ...
        "or successful access unless the supplied evidence explicitly establishes it. Do not invent "
        "numeric confidence percentages; use qualitative confidence such as low, moderate, or high and "
        "briefly state what evidence supports that level. Clearly separate observations from hypotheses. "
        "Treat brute force, scanning, exploitation, and coordinated activity as hypotheses unless the "
        "evidence directly demonstrates them. Recommend verification and context collection before blocking, "
        "credential changes, containment, or other disruptive action. Never provide destructive or offensive "
        "actions.\n\n"
