...
class TrendTracker:
    ...
    def _classify(line: str) -> tuple[bool, bool, bool]:
        text = line.lower()
        failed_login = any(marker in text for marker in ("failed password", "authentication failure", "login failed", "invalid user", "failed login"))
        error = any(marker in text for marker in (" error ", "error:", "failed", "failure", "exception", "timeout"))
        firewall = any(marker in text for marker in ("ufw block", "firewall block", "blocked connection", "action=block", "action=deny", " denied "))
    ...
    def _append_bucket(self, stamp: float, failed: int, errors: int, firewall: int) -> None:
        ...
        while len(self._events) > self.max_buckets:
            ...
            if not self._events:
                self._events.append(oldest); break
            next_bucket = self._events.popleft()
            self._events.appendleft((next_bucket[0], oldest[1] + next_bucket[1], oldest[2] + next_bucket[2], oldest[3] + next_bucket[3]))

    def ingest(self, lines: list[str], now: float | None = None) -> TrendSnapshot:
        stamp = time.monotonic() if now is None else now
        self._trim(stamp)
        failed = errors = firewall = 0
        for line in lines:
            auth, error, blocked = self._classify(line); failed += int(auth); errors += int(error); firewall += int(blocked)
        self._append_bucket(stamp, failed, errors, firewall); self._trim(stamp)
        current = self._rates(); metrics: list[TrendMetric] = []
        minimums = {"Failed logins": 5.0, "Errors": 6.0, "Firewall blocks": 5.0}
        for name, value in current.items():
            baseline = self._baseline.get(name, value); ratio = value / baseline if baseline > 0 else (999.0 if value > 0 else 1.0)
            ...
            metrics.append(TrendMetric(name, value, baseline, ratio, state)); self._baseline[name] = value if name not in self._baseline else (self.alpha * value + (1.0 - self.alpha) * baseline)
    ...
    def _rates(self) -> dict[str, float]:
        ...
        return {"Failed logins": sum(a for _, a, _, _ in self._events) * scale, "Errors": sum(e for _, _, e, _ in self._events) * scale, "Firewall blocks": sum(b for _, _, _, b in self._events) * scale}
    ...
    def snapshot(self, now: float | None = None) -> TrendSnapshot:
        ...
        for name, value in current.items():
            baseline = self._baseline.get(name, value); ratio = value / baseline if baseline > 0 else (999.0 if value > 0 else 1.0); minimum = 5.0 if name != "Errors" else 6.0
...
def _spark(current: float, baseline: float, width: int = 14) -> Text:
    maximum = max(current, baseline, 1.0)
    current_n = min(width, round((current / maximum) * width)); base_n = min(width, round((baseline / maximum) * width))
    text = Text()
    for index in range(1, width + 1):
        if index <= current_n and index <= base_n: text.append("■", style=ACCENT)
        elif index <= current_n: text.append("■", style=WARNING)
        elif index <= base_n: text.append("·", style=MUTED)
        else: text.append("·", style="#394447")
    return text


def render_trends(snapshot: TrendSnapshot, metric_names: tuple[str, ...] | None = None) -> Panel:
    allowed = set(metric_names or ()); metrics = [m for m in snapshot.metrics if not allowed or m.name in allowed]
    table = Table(expand=True, box=None, padding=(0, 1))
    table.add_column("SIGNAL", ratio=2, style=NEUTRAL)
    table.add_column("ACTIVITY", width=16)
    table.add_column("CURRENT", width=10, justify="right")
    table.add_column("BASELINE", width=10, justify="right", style=MUTED)
    table.add_column("DELTA", width=8, justify="right")
    table.add_column("STATE", width=10)
    for metric in metrics:
        ratio = f"{metric.deviation_ratio:.1f}x" if metric.deviation_ratio < 100 else ">99x"; style = _state_style(metric.state)
        table.add_row(metric.name, _spark(metric.current_per_minute, metric.baseline_per_minute), Text(f"{metric.current_per_minute:.1f}/m", style=style if metric.state != "NORMAL" else INFO), f"{metric.baseline_per_minute:.1f}/m", Text(ratio, style=style if metric.state != "NORMAL" else MUTED), Text(metric.state, style=style))
    if not metrics: table.add_row("No profile metrics", Text("··············", style=MUTED), "0.0/m", "0.0/m", "1.0x", Text("NORMAL", style=SUCCESS))
    legend = Text("■ current / overlap   · baseline / remaining", style=MUTED)
    return Panel(Group(table, Text(""), legend), title=Text(f" RATE & BASELINE  ·  {snapshot.window_seconds}s WINDOW ", style=f"bold {ACCENT}"), title_align="left", box=box.ROUNDED, border_style=ACCENT_SOFT, padding=(0, 1))
