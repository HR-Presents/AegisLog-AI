...
def _incident_rows(data: DashboardData) -> str:
    if not data.incidents:
        return '<div class="empty">No correlated incidents were retained.</div>'
    rows: list[str] = []
    for item in data.incidents:
        css = "danger" if item.severity in {"CRITICAL", "HIGH"} else "warning" if item.severity == "MEDIUM" else "good"
        evidence = "<br>".join(f"<code>{escape(line)}</code>" for line in item.evidence)
        rows.append(
            f"<tr><td><b>INC-{escape(item.id.upper()[:8])}</b></td><td><span class='pill {css}'>{escape(item.severity)}</span></td><td>{escape(item.category)}</td><td>{escape(item.title)}</td><td>{item.count}</td><td>{evidence}</td></tr>"
        )
    return "".join(rows)
...
def _anomaly_rows(data: DashboardData) -> str:
    if not data.anomalies:
        return '<div class="empty">No anomaly classes met the retained scoring threshold.</div>'
    return "".join(
        f"<tr><td><b>{escape(item.key)}</b></td><td>{item.score:.1f}</td><td>{escape(item.reason)}</td></tr>"
...
def build_html_report(data: DashboardData) -> str:
    ...
    confidence = max(0, 100 - min(70, elevated * 7 + len(data.anomalies) * 3))
    incident_share = _pct(len(data.incidents), max(total_findings, 1))
...
<section class="card" id="visuals"><div class="section-kicker">Visual analytics</div><h2>Investigation Visuals</h2><div class="mini-grid"><div class="mini-card"><strong>{confidence}%</strong><span>Coverage confidence indicator</span></div><div class="mini-card"><strong>{elevated}</strong><span>Medium+ retained findings</span></div><div class="mini-card"><strong>{incident_share:.1f}%</strong><span>Incident-to-finding ratio</span></div></div><div class="chart-grid" style="margin-top:18px"><div class="chart-card"><h3>Severity Distribution</h3>{severity_html}</div><div class="chart-card"><h3>Service Activity</h3>{service_html}</div><div class="chart-card"><h3>Finding Categories</h3>{category_html}</div><div class="chart-card"><h3>Log Level Mix</h3>{level_html}</div></div></section>
