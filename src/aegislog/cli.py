...
def threats(path: Path = typer.Argument(..., exists=True, dir_okay=False)) -> None:
    ...
    for finding in security: console.print(f"[{finding.severity}] {finding.title}\n  {finding.evidence}\n  Next: {finding.recommendation}")
...
def anomalies(path: Path = typer.Argument(..., exists=True, dir_okay=False)) -> None:
    ...
    for item in results[:30]: console.print(f"[bold]{item.score:>5.1f}[/bold]  {item.key}  {item.reason}")
...
def ask_log(question: str, path: Path = typer.Argument(..., exists=True, dir_okay=False), local: bool = False) -> None:
    ...
    try:
        ...
        console.print(f"AI provider unavailable: {exc}\nFalling back to local analysis."); console.print(Panel(local_answer(context), title=f"Investigation: {question}"))
...
def collect(source: str = typer.Argument(..., help="journal or docker"), target: str = "", lines: int = 300, output: Path = Path("aegislog-collected.log")) -> None:
    ...
    except CollectorError as exc: console.print(f"Collection failed: {exc}"); raise typer.Exit(1)
    output.write_text("\n".join(result.lines) + "\n", encoding="utf-8"); console.print(f"Collected {len(result.lines)} lines from {result.source} -> {output}")
    ...
    try:
        with path.open("r", encoding="utf-8", errors="replace") as handle:
            ...
            while True:
                ...
                for finding in findings: console.print(f"[{finding.severity}] {finding.title} | {finding.evidence}")
                if not findings and event.level in {"error", "warning"}: console.print(f"[{event.level.upper()}] {event.message}")
...
def config(provider: str = "none", model: str = "", base_url: str = "") -> None:
    ...
    path = save_config({"ai_provider": provider.lower(), "model": model, "base_url": base_url or None}); console.print(f"Configuration saved to {path}")


@app.command()
def doctor() -> None:
    """Check the local AegisLog runtime."""
    cfg = load_config(); console.print(f"AegisLog AI {__version__}\nPython: {platform.python_version()}\nPlatform: {platform.platform()}"); console.print("Local detection engine: ready"); console.print(f"AI provider preference: {cfg.get('ai_provider', 'none')}  model: {cfg.get('model', '') or '(default)'}")
...
def scan(path: Path = typer.Argument(Path("/var/log"))) -> None:
    ...
    files = [p for p in path.rglob("*") if p.is_file() and (p.suffix in {".log", ".txt"} or "log" in p.name.lower())][:100]; console.print(f"Scanning {len(files)} candidate log files under {path}")
    for file in files:
