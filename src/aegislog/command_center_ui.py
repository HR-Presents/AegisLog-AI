...
def _header(title: str, source: str, profile: str, risk: str, *, subtitle: str = "") -> Panel:
    ...
    grid.add_row(source_line, mode); grid.add_row(Text(subtitle or "LOCAL | READ-ONLY | DETERMINISTIC", style=MUTED), Text("HR-PRESENTS", style=f"bold {ACCENT}"))
...
def _mini_bar(count: int, maximum: int, width: int = 16, *, style: str = ACCENT) -> Text:
    maximum = max(1, maximum); filled = max(1, round((count / maximum) * width)) if count else 0
    text = Text("=" * filled, style=style); text.append(" " * (width - filled)); return text
...
def _distribution(title: str, values: Counter[str], *, compact: bool, semantic: bool = False) -> Panel:
    ...
    table = Table.grid(expand=True, padding=(0, 1)); table.add_column(ratio=1, overflow="crop"); table.add_column(width=18); table.add_column(width=9, justify="right")
...
def _recent_findings(findings, profile_label: str, *, compact: bool) -> Panel:
    ...
    if compact: table.add_column("FINDING", ratio=1, overflow="fold")
        ...
        table.add_column("SEV", width=9); table.add_column("CATEGORY", width=14, overflow="crop"); table.add_column("FINDING", width=26, overflow="fold"); table.add_column("EVIDENCE", ratio=1, overflow="fold")
...
def _source_activity(state, *, compact: bool) -> Panel:
    ...
    table = Table(expand=True, box=None, padding=(0, 1)); table.add_column("SOURCE", ratio=2, overflow="crop"); table.add_column("ACTIVITY", width=18); table.add_column("STATUS", width=10, no_wrap=True); table.add_column("EVENTS", width=8, justify="right")
...
def render_multisource_command_center(state) -> RenderableType:
    ...
    sections.extend((Text(""), render_trends(trend, profile.trend_metrics), Text(""), _alerts(state, compact=compact), Text(""), Panel(Text(f"Multi-source monitoring active | {state.total_bytes:,} bytes across {len(state.sources)} sources | no remediation", style=MUTED), title=Text(" SOC STATUS ", style=f"bold {ACCENT}"), title_align="left", box=box.ROUNDED, border_style=ACCENT_SOFT)))
    return Group(*sections)
